...
function run {
  ...
  wave=$source
  if [[ $param1 = cpu ]]; then 
    wave=../_testbench/cpu_tb$n
    source=$source$n
  fi
   

  echo "== elaboration ${param1}_tb =="
  ../_bin/ghdl-0.33-x86_64-linux/bin/ghdl -e --workdir=../_lib -P../_lib $param1"_tb"
  echo
  ...
  if [[ $param1 = cpu ]]; then rm ../_testbench/cpu_tb.num ; fi
