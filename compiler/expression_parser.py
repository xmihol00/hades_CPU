...
from constructs import IntermediateResult, Variable, Constant
...
class ExpressionParser:
    ...
    def add_variable_operand(self, operand: str|Variable):
    ...
    def add_opened_bracket(self):
        if ExpressionParserStates.UNARY_OPERATOR_OR_OPERAND_OR_OPENED_BRACKET == self.state:
            self.bracket_stack.append((self.current_precedence, self.operator_stack, self.operand_stack))
            self.operator_stack = []
            self.operand_stack = []
            self.current_precedence = -1
        else:
    ...
    def add_closed_bracket(self):
        if ExpressionParserStates.BINARY_OPERATOR_OR_CLOSED_BRACKET == self.state:
            ...
            if len(self.bracket_stack) > 0:
                ...
                self.current_precedence, self.operator_stack, self.operand_stack = self.bracket_stack.pop()
